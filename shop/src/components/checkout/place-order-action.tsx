...
export const PlaceOrderAction: React.FC<{
  ...
  const handlePlaceOrder = () => {
    console.log('aaa customer_contact', customer_contact);
    if (!customer_contact) {
      setErrorMessage(t('error-contact-number-is-required'));
      return;
    }
    if (!recipient_name) {
      setErrorMessage(t('error-contact-name-of-recipient-is-required'));
      return;
    }
    if (!recipient_phone) {
      setErrorMessage(t('error-phone-of-recipient-is-required'));
      return;
    }
    if (!use_wallet_points && !payment_gateway) {
      setErrorMessage(t('error-gateway-is-required'));
      return;
    }
