...
export const orderRecipientNoteAtom = atom(
  (get) => get(checkoutAtom).recipient_note,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, recipient_note: data });
  }
);
export const orderRecipientNameAtom = atom(
  (get) => get(checkoutAtom).recipient_name,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, recipient_name: data });
  }
);
export const orderRecipientPhoneAtom = atom(
  (get) => get(checkoutAtom).recipient_phone,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, recipient_phone: data });
  }
);

export const orderMessageAtom = atom(
  (get) => get(checkoutAtom).message,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, message: data });
  }
);
export const orderMessageFromAtom = atom(
  (get) => get(checkoutAtom).message_from,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, message_from: data });
  }
);
export const orderMessageToAtom = atom(
  (get) => get(checkoutAtom).message_to,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, message_to: data });
  }
);
